...
```
ユーザー: この機能のコードレビューをして

Orchestrator:
  → Development Team / Code Reviewer に振り分け
  → Code Reviewer がレビュー実行
  → 結果を返却
```
...
以下のエージェントは直接コマンド呼び出しではなく、Orchestratorからの委譲で動作する内部エージェントです:
